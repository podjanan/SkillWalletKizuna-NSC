...
export function buildVirtualBilingualSongs() {
  return {
    ...
    nameActivity: 'Sing Together',
    category: 'PHYSICAL',
    descriptionActivity: 'กิจกรรม Sing Together ร้องเพลงเต้นด้วยกันและคอร์ดกีต้าร์สำหรับผู้ปกครอง',
    createdAt: new Date().toISOString(),
    responses: 0,
    id: 'bilingual-songs',
    name: 'Sing Together',
    ...
    description: 'กิจกรรม Sing Together ร้องเพลงเต้นด้วยกันและคอร์ดกีต้าร์สำหรับผู้ปกครอง',
    videoUrl: '',
...
export async function shouldInjectBilingualSongs(category: string | null, ownedBy: string | null) {
  if (ownedBy) return null;
  const normalized = (category || '').trim().toUpperCase();
  if (normalized && normalized !== 'ALL' && normalized !== 'PHYSICAL' && category !== 'ด้านร่างกาย' && normalized !== 'LANGUAGE') {
    return null;
  }
